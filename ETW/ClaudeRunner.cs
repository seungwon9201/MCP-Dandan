...
namespace CursorProcessTree
{
    public static class ClaudeRunner
    {
        public static void Run(string targetProcess)
        {
            ...
            {
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Claude") + Path.DirectorySeparatorChar,
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AnthropicClaude") + Path.DirectorySeparatorChar
            };
            ...
            using var session = new TraceEventSession("ClaudeObserverSession");
